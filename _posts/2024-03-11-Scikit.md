...
교차 검증의 한 방법으로, KFold 방법에 대해 알아보겠습니다. 전체 데이터를 K개의 부분 집합으로 나누고, 이 중 하나의 폴드를 테스트 세트로 사용하고 나머지를 학습 세트로 사용하여 모델을 평가하는 이 과정을 K번 반복하여, 각 폴드에 대한 모델 성능을 평가한 뒤, 이들의 평균값을 최종 모델 성능으로 사용합니다.
...
정답은 max_depth와 min_samples_split 입니다.
